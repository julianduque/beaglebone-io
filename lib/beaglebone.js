...
function i2cDevice(address) {
  var device = _i2cDevices[address];

  if (device === undefined) {
    device = _i2cDevices[address] = i2c.openSync(1);
  }
...
BeagleBone.prototype.i2cWrite = function(address, cmdRegOrData, inBytes) {
  ...
  if (buffer.length) {
   i2cDevice(address).i2cWriteSync(address, buffer.length, buffer);
  }
...
BeagleBone.prototype.i2cWriteReg = function(address, register, value) {
  ...
  i2cDevice(address).writeByteSync(address, register, value);

  return this;
};

BeagleBone.prototype._i2cRead = function(continuous, address, register, bytesToRead, callback) {
  ...
  setTimeout(function read() {
    ...
    if (register !== null) {
      device.readI2cBlock(address, register, bytesToRead, data, afterRead);
    } else {
      device.i2cRead(address, bytesToRead, data, afterRead);
    }
  }.bind(this), _i2cDelay);
