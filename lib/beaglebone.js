...
i2cWrite(address, cmdRegOrData, inBytes)                writeI2cBlockSync(addr, cmd, length, buffer)
...
BeagleBone.prototype.i2cWrite = function(address, cmdRegOrData, inBytes) {
  ...
  if (arguments.length === 3 &&
    ...
    buffer = new Buffer(inBytes);
    _i2c.writeI2cBlockSync(address, cmdRegOrData, buffer.length, buffer);
    return this;
  }
...
BeagleBone.prototype.i2cRead = function(address, register, bytesToRead, handler) {
  ...
  this.on(event, readLoop);

  function readLoop() {
    _i2c.i2cRead(address, bytesToRead, buffer, function(i2cError, i2cNumBytes, buffer) {
      if (i2cError) {
        this.emit("error", i2cError);
        return;
      }
