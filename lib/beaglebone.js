...
var i2cBuffer = new Buffer(255); // maximum size of PMBus frame
var i2cCopyCount;
var i2c1;
...
BeagleBone.prototype.sendI2CConfig = function() {
  if (i2c1 === undefined) {
    i2c1 = i2c.openSync(1, function(i2cError) {
      if (i2cError) {
        throw new Error("I2COpen Error" );
        console.error("I2COpen ERROR: error = " + i2cError);
      }
    });
  }
  return this;
...
BeagleBone.prototype.sendI2CWriteRequest = function(address, bytes) {
  for (i2cCopyCount = 0; i2cCopyCount < bytes.length; i2cCopyCount++) {
    i2cBuffer[i2cCopyCount] = bytes[i2cCopyCount];
  }

  i2c1.i2cWrite(address, bytes.length, i2cBuffer, function(i2cError, i2cNumBytes, i2cBuffer) {
    if (i2cError) {
      throw new Error("I2CWrite Error");
      console.error("I2CWrite ERROR: error = " + i2cError);
    }
  });
  return this;
};
...
BeagleBone.prototype.sendI2CReadRequest = function(address, length, cb) {
  i2c1.i2cRead(address, length, i2cBuffer, function(i2cError, i2cNumBytes, i2cBuffer) {
    if (i2cError) {
      throw new Error("I2CRead Error");
      console.error("I2CReadERROR: error = " + i2cError);
    }
    cb(i2cBuffer);
